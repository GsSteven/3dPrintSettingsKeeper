...
class NewDesign extends React.Component {
  ...
  setAdhesionType(e) {
    ...
    switch (value) {
      ...
      case "none":
        ...
        break;
      case "":
        this.setState({ buildPlateAdhesionType: "" });
        break;
      default:
  ...
  getAdhesionType() {
    switch (this.state.buildPlateAdhesionType) {
      ...
      case "none":
        break;
      case "":
        return <div className="subclass"></div>;
      default:
  ...
  getFiles() {
    const files = this.state.file.map((file) => {
      return (
        <li className="listFile" key={file.name}>
          {file.name}
          <img
            src={removeFileIcon}
            id={file.name}
      ...
      if (payLoad.img) {
        ...
        await axios
          .post(`/api/upload`, imgFormData, {
            headers: { "Content-Type": this.state.img.type },
            params: mergeTitle,
          })
          .then((response) => {
        ...
        <form onSubmit={this.handleSubmit} autoComplete="off">
          ...
          <div className="uploadWrapper">
            <label htmlFor="img" className="mainLabel">
              ...
              <div id="printImg">
                <img
                  src={this.state.tempImg ? this.state.tempImg : defaultImg}
                  alt=""
                ></img>
              </div>
            </label>
